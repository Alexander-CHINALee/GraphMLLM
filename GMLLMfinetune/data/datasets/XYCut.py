...
def xy_cut(bboxes, direction="x", last=False):
    result = []
    K = len(bboxes)
    indexes = range(K)
    if len(bboxes) <= 0:
    ...
    if last and curr==0:
        result += sorted_ids[curr:idx + 1]
        return result
    rel_res = xy_cut(sorted_boxes[curr:idx + 1], next_dir, last=True)
    result += [sorted_ids[i + curr] for i in rel_res]
    return result
...
if __name__ == '__main__':
    ...
    res_idx = augment_xy_cut(bboxes, direction="y")
    print(res_idx)
    res_idx = augment_xy_cut(bboxes, direction="x")
    print(res_idx)
